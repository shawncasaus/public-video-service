...
async fn test_no_header_generates_uuid() {
    ...
    assert!(request_id_header.is_some(), "Response should include x-request-id header");
    
    // Should be a valid UUID
    let request_id = request_id_header.unwrap().to_str().unwrap();
    let uuid = Uuid::parse_str(request_id);
    assert!(uuid.is_ok(), "Request ID should be a valid UUID: {}", request_id);
}
...
async fn test_health_endpoint_gets_request_id() {
    ...
    // Should have x-request-id header
    let request_id_header = response.headers().get("x-request-id");
    assert!(request_id_header.is_some(), "Health endpoint should include x-request-id header");
...
async fn test_cors_exposes_request_id() {
    ...
    assert!(expose_headers_str.contains("x-request-id"), 
            "Should expose x-request-id header: {}", expose_headers_str);
}
...
async fn test_different_requests_get_different_uuids() {
    ...
    assert_ne!(id1, id2, "Different requests should get different request IDs");
}
